...
def get_transformer_blocks(model):
    """
    Return the container of transformer blocks across many HF architectures.
    Works for LLaMA/Mistral, GPT-2/Falcon, GPT-NeoX, MPT, Qwen, etc.
    """
    ...
    # find the actual block list
    for name in ("layers", "h", "blocks"):
        if hasattr(core, name):
            return getattr(core, name)

    # last resort: some models use .decoder.layers (e.g., BART-ish)
    if hasattr(core, "decoder") and hasattr(core.decoder, "layers"):
        return core.decoder.layers

    raise AttributeError(
        f"Could not find transformer blocks on {type(model).__name__}. "
        f"Tried: model/transformer/gpt_neox/base_model + layers/h/blocks/decoder.layers"
    )
