...
def eval_zero_shot(
    ...
    model.save_pretrained(f"./temp_model/{model_name}")
    tokenizer.save_pretrained(f"./temp_model/{model_name}")
    task_names = task_list#pattern_match(task_list, tasks.ALL_TASKS)
    model_args = f"pretrained=./temp_model/{model_name}"
    if use_accelerate:
        model_args = (
            f"pretrained=./temp_model/{model_name},use_accelerate=True"
        )
    ...
    # clean up
    os.system(f"rm -rf ./temp_model/{model_name}")
